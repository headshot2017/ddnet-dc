...
#if !defined(CONF_PLATFORM_MACOSX)
	#if defined(CONF_FAMILY_KOS)
	void semaphore_init(SEMAPHORE *sem) { sem_init(sem, 1); }
	void semaphore_wait(SEMAPHORE *sem) { sem_wait(sem); }
	void semaphore_signal(SEMAPHORE *sem) { sem_signal(sem); }
...
int net_set_non_blocking(NETSOCKET sock)
{
	...
	if(sock.ipv4sock >= 0)
	{
		...
		fcntl(sock.ipv4sock, F_SETFL, O_NONBLOCK);
#endif
	}
...
{
	snprintf(path, max, "/sd/ddnet/user");
	return 0;
}
...
int _fs_rename(const char *oldname, const char *newname)
{
#if defined(CONF_FAMILY_WINDOWS)
...
#elif defined(CONF_FAMILY_UNIX) && !defined(CONF_FAMILY_KOS)
	char* argv[2];
	argv[0] = (char*) file;
	argv[1] = NULL;
	pid_t pid = fork();
